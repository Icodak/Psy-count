...
$topic_uuid = $_GET['topic_uuid'];
    try {
        ...
        $req =  $dbco->prepare('SELECT M.ID_message, M.ID_utilisateur, M.isHidden, M.isModified, M.creationDate, M.message, M.ID_topic, U.nom AS userLastName, U.prenom AS userFirstName, u.images AS userProfile, u.permission_lvl AS userRank FROM messages AS M LEFT JOIN utilisateur AS U ON M.ID_utilisateur = U.ID_Utilisateur WHERE M.ID_topic = :topic_uuid ORDER BY M.creationDate;');
